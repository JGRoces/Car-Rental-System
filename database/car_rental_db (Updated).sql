-- ============================================================
--  CAR RENTAL SYSTEM — Full Database Setup
--  File   : car_rental_db.sql
--  Place  : database/car_rental_db.sql
--
--  HOW TO RUN:
--    Option A (MySQL CLI):
--      mysql -u root -p < car_rental_db.sql
--
...
-- ============================================================
--  2. USER ACCOUNT FOR THE JAVA APP
--  The Java app connects as carrentaluser / carrentalpass.
--  Root credentials are never stored in application code.
-- ============================================================
...
-- ------------------------------------------------------------
--  users  —  Base account for BOTH Admin and Customer actors.
--            The `role` column determines which dashboard
--            the user is routed to after login.
-- ------------------------------------------------------------
CREATE TABLE users (
...
--            image_path stores the FILENAME only
--            (e.g. "toyota-vios.png").
--            The Java app resolves it to assets/images/<filename>.
-- ------------------------------------------------------------
CREATE TABLE cars (
...
--              total_amount is calculated by RentalService
--              (days x daily_rate) before inserting.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
--  payments  —  One payment record per rental.
-- ------------------------------------------------------------
...
('Admin User', 'admin@carrentals.com', 'admin123', 'ADMIN');

-- ------------------------------------------------------------
--  Customer accounts  (password: customer123 for all)
-- ------------------------------------------------------------
INSERT INTO users (full_name, email, password, role) VALUES
...
-- Customer profiles  (user_id 2, 3, 4 from inserts above)
INSERT INTO customers (user_id, phone_number, address, license_number) VALUES
(2, '09171234567', '123 Rizal St, Manila',              'LIC-2024-001'),
(3, '09281234567', '456 Bonifacio Ave, Quezon City',    'LIC-2024-002'),
(4, '09391234567', '789 Mabini St, Makati',             'LIC-2024-003');

-- ------------------------------------------------------------
--  Cars
--  image_path must match the actual filename in assets/images/
--  Set to NULL if no image file exists yet (emoji fallback).
-- ------------------------------------------------------------
INSERT INTO cars
    (brand, model, year, plate_number, category, transmission, fuel_type, seat_capacity, daily_rate, status, image_path, color)
VALUES
-- Toyota
('Toyota',     'VIOS',     2023, 'ABC-1234', 'Sedan',  'Automatic', 'Gasoline', 5, 1600.00, 'AVAILABLE',   'Toyota VIOS - 2024 (Red).jpg',          'Red'),
...
('Mitsubishi', 'Montero',  2022, 'EFG-1111', 'SUV',    'Automatic', 'Diesel',   7, 2600.00, 'AVAILABLE',   'Mitsubishi Montero - 2023 (Gray).jpg',  'Gray'),
...
-- ------------------------------------------------------------
--  Sample rentals
--  customer_id 1 = Juan, 2 = Maria, 3 = Carlos
--  car_id      5 = Fortuner, 1 = Vios, 9 = Mirage
-- ------------------------------------------------------------
INSERT INTO rentals (customer_id, car_id, start_date, end_date, total_amount, status) VALUES
(1, 5, '2026-03-01', '2026-03-05', 11200.00, 'COMPLETED'),
(2, 1, '2026-03-10', '2026-03-12',  3200.00, 'ACTIVE'),
(3, 9, '2026-03-15', '2026-03-18',  3600.00, 'PENDING');
...
-- ============================================================
--  5. VERIFY — Quick row counts to confirm setup worked
--  Expected: users=4, customers=3, cars=13, rentals=3, payments=3
-- ============================================================
